...
namespace MAIN {
    ...
    private:
        struct Node {
            // Main data and pointers into left and right child elements
            int key;
            T data;
            Node* left, * right;
            ...
            Node(const int& key, T data) :key(key), left(nullptr), right(nullptr), data(data){}
        ...
        ////////////////////////////////////////////////
        // BST additional functions for using them in 
    ...
    public:
        ...
        // Create BST including array of starts elements
        template <typename ...Args>
        BinTree(const Node& item, Args...more) {
            this->insert(item, more...);
        }

        ////////////////////////////////////////////////
        // BST Delete and Destructors
        ////////////////////////////////////////////////
        ...
        void insert(const int key, T data) {
            if (!root) {
        ...
        template <typename ...Args>
        void insert(const Node& item, Args... more) {
            if (!root) {
                root = new Node(item.key, item.data);
            }
            else {
                Node* p = root;
                for (;;) {
                    if (item.key == p->key) return;
                    Node*& pchild = item.key < p->key ? p->left : p->right;
                    if (!pchild) {
                        pchild = new Node(item.key, item.data);
                        break;
                    }
                    p = pchild;
                }
